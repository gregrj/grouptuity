...
class BasicTipCalcViewModel(app: Application): UIViewModel(app) {
    ...
    private val mTaxInput = MutableStateFlow(Pair(repository.prefDefaultTaxPercent.value.toDouble(), true))
    private val mTipInput = MutableStateFlow(Pair(repository.prefDefaultTipPercent.value.toDouble(), true))
    private val mTaxTipped = MutableStateFlow(repository.prefTaxIsTipped.value)
    private val mDiscountReducesTip = MutableStateFlow(repository.prefDiscountsReduceTip.value)
    ...
    private val taxPercent = MutableStateFlow(repository.prefDefaultTaxPercent.value.toDouble())
    private val taxAmount = MutableStateFlow(0.0)
    private val subtotalWithDiscountsAndTax = MutableStateFlow(0.0)
    private val tipPercent = MutableStateFlow(repository.prefDefaultTipPercent.value.toDouble())
    ...
    fun reset() {
        ...
        mTaxInput.value = Pair(repository.prefDefaultTaxPercent.value.toDouble(), true)
        mTipInput.value = Pair(repository.prefDefaultTipPercent.value.toDouble(), true)
        mTaxTipped.value = repository.prefTaxIsTipped.value
        mDiscountReducesTip.value = repository.prefDiscountsReduceTip.value
    }
}